...
import frc.robot.lib.trajectory.TrajectoryPoint;
...
public class SplineGenerator {
     ...
     */
    public static List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> parameterizeSpline(Spline s, List<? extends ImprovedRotation2d> headings, double maxDx, double maxDy, double
            maxDTheta, double t0, double t1) {
        List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> rv = new ArrayList<>();
     ...
     */
    public static List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> parameterizeSpline(Spline s, List<? extends ImprovedRotation2d> headings) {
        return parameterizeSpline(s, headings, kMaxDX, kMaxDY, kMaxDTheta, 0.0, 1.0);
    }

    public static List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> parameterizeSpline(Spline s, List<? extends ImprovedRotation2d> headings, double maxDx, double maxDy, double maxDTheta) {
        return parameterizeSpline(s, headings, maxDx, maxDy, maxDTheta, 0.0, 1.0);
    }

    public static List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> parameterizeSplines(List<Spline> splines, List<? extends ImprovedRotation2d> headings) {
        return parameterizeSplines(splines, headings, kMaxDX, kMaxDY, kMaxDTheta);
    }

    public static List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> parameterizeSplines(List<? extends Spline> splines, List<? extends ImprovedRotation2d> headings, double maxDx, double maxDy,
                                                                double maxDTheta) {
        List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> rv = new ArrayList<>();
        ...
        for (int i = 0; i < splines.size(); i++) {
            ...
            List<ImprovedRotation2d> spline_rots = new ArrayList<>();
            spline_rots.add(headings.get(i));
            spline_rots.add(headings.get(i+1));

            List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> samples = parameterizeSpline(s, spline_rots, maxDx, maxDy, maxDTheta);
            samples.remove(0);
            rv.addAll(samples);
        }
        return rv;
    ...
    private static void getSegmentArc(Spline s, List<? extends ImprovedRotation2d> headings, List<TrajectoryPoint<Pose2dWithCurvature, ImprovedRotation2d>> rv,  double t0, double t1, double maxDx,
                                      double maxDy,
                                      double maxDTheta, double totalTime) {
        Translation2d p0 = s.getPoint(t0);
        Translation2d p1 = s.getPoint(t1);
        ImprovedRotation2d r0 = s.getHeading(t0);
        ImprovedRotation2d r1 = s.getHeading(t1);
        Pose2d transformation = new Pose2d(new Translation2d(p0, p1).rotateBy(r0.inverse()), r1.rotateBy(r0.inverse()));
        ImprovedTwist2d twist = Pose2d.log(transformation);
        ...
        if (twist.dy > maxDy || twist.dx > maxDx || twist.dtheta > maxDTheta) {
            ...
            // Interpolate heading
            ImprovedRotation2d diff = headings.get(1).rotateBy(headings.get(0).inverse());
            if (diff.getRadians() > Math.PI) {
                diff = diff.inverse().rotateBy(ImprovedRotation2d.fromRadians( Math.PI));
            }
            ImprovedRotation2d interpolated_heading = headings.get(0).rotateBy(diff.times(t1 / totalTime));
