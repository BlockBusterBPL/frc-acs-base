...
public class AutoAlignMotionPlanner {
    ...
    private Pose2d poseInverse(Pose2d pose) {
        Rotation2d rotationInverse = pose.getRotation().times(-1);
        return new Pose2d(pose.getTranslation().rotateBy(rotationInverse), rotationInverse);
    }

    private Pose2d poseTransformBy(Pose2d current, Pose2d other) {
        return new Pose2d(
            current.getTranslation().plus(other.getTranslation().rotateBy(current.getRotation())),
            current.getRotation().rotateBy(other.getRotation())
        );
    }
