    ...
    public synchronized void setDesiredState(ArmState desiredState, ArmState currentState) {
        ...
        if (currentState.getExtend() > kMaxExtensionWhileStowed && currentState.getTilt() < kMinTiltForExtension) {
            ...
            mIntermediateStateQueue.add(
                ArmState.generateWithFailsafeParameters(kMinTiltForExtension + 0.1, currentState.extend, currentState.wrist)
            );
            mIntermediateStateQueue.add(
                ArmState.generateWithFailsafeParameters(kMinTiltForExtension + 0.1, currentState.extend, 0)
            );
            mIntermediateStateQueue.add(
                ArmState.generateWithFailsafeParameters(kMinTiltForExtension + 0.1, 0, 0)
            );
            mIntermediateStateQueue.add(
        ...
        // if target tilt is lower than current tilt, we should fully retract before we can tilt
